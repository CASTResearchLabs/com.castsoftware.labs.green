...
*  The *GreenIT Index* is a Business Criterion that aggregates Quality Rules from existing high-impact Efficiency-related Technical Criteria
...
To avoid computing the GreenIT index for applications that are not candidate to moving to the cloud:
* open CAST Management Studio
* edit the Dashboard Service(s) you are publishing the Snapshots of the N/A application(s)
* select the Assessment Model tab
* create an exclusion for the GreenIT index and the N/A application(s)
